...
export async function getRelayerCapabilities(): Promise<RelayerCapabilities> {
  ...
  _capabilities = result as RelayerCapabilities;
...
export async function getRelayFee(
  paymentToken: `0x${string}` = (process.env.NEXT_PUBLIC_USDC_ADDRESS as `0x${string}`) || '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913'
): Promise<FeeData> {
  const response = await fetch(RELAYER_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
