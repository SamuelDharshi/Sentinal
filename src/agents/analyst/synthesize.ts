...
`;
  ...
  try {
    ...
    console.error('[Analyst] Venice AI synthesis failed, using fallback:', error);

    // Fallback: generate a card from the raw data
    synthesized = generateFallbackCard(batch, userBrief);
  }

  // Build delegation trace chips — the signature SENTINEL design element
  ...
  const sources: Source[] = batch.map((r) => ({
    ...
    url: `https://basescan.org/tx/${r.txHash || '0x0'}`,
    type: r.cost > 0 ? 'x402' : 'free',
    cost: r.cost,
  }));

  const analystTxHash = `0x${crypto.randomBytes(32).toString('hex')}` as `0x${string}`;
...
function generateFallbackCard(batch: ScoutResult[], userBrief: string): {
  headline: string;
  summary: string;
  urgency: 'low' | 'medium' | 'high';
  actionSuggested: string;
  ...
  return {
    headline: `Intelligence from ${firstResult.source}: ${firstResult.query.slice(0, 60)}`,
    summary: `Scout collected ${batch.length} data points from ${batch.map((r) => r.source).join(', ')}. ` +
      `Relevant to your brief: "${userBrief.slice(0, 100)}". Review the raw data for detailed insights.`,
    urgency: 'medium',
    actionSuggested: 'Review the collected intelligence and identify specific action items for your strategy.',
    sourceCount: batch.length,
  };
}
